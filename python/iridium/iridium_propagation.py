# ------------------------------------------------------------------------------
# iridium + TOLOSAT ORBIT PROPAGATION SCRIPT (SEGMENTED)
# ------------------------------------------------------------------------------
# This script propagates Tolosat and iridium satellites using TudatPy in segments
# (~94.4 min/orbit) to improve long-term integration stability.
#
## Initial States:
# - iridium satellite states are extracted from TLEs (≤5 days old) via `galileo_TLE_sync.py`
...
from iridium_TLE_sync import iridium_states_synced, iridium_names, start_datetime
...
mission_duration_days = 30  # total simulation duration in days
...
# Create iridium satellites
for iridium in iridium_names:
...
acceleration_settings = {
    "Tolosat": {
        ...
        "Earth": [propagation_setup.acceleration.spherical_harmonic_gravity(10, 10), propagation_setup.acceleration.aerodynamic()],
        "Moon": [propagation_setup.acceleration.point_mass_gravity()],
        "Jupiter": [propagation_setup.acceleration.point_mass_gravity()],
    }
}
for iridium in iridium_names:
    acceleration_settings[iridium] = {"Earth": [propagation_setup.acceleration.spherical_harmonic_gravity(2, 0)]}

all_spacecraft = ["Tolosat"] + iridium_names
...
iridium_states = iridium_states_synced[["x", "y", "z", "vx", "vy", "vz"]].to_numpy()
initial_state = np.concatenate(([Tolosat_initial_state], iridium_states)).flatten()
...
print("Propagating iridium + Tolosat in segments...")
...
# Create new folder with auto-numbering
base_path = "iridium_states"
...
# Save epochs, sun direction, spacecraft states
...
print(f"Done! Saved all results in folder: iridium_states/{next_index}")
