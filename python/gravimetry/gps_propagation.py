# ------------------------------------------------------------------------------
# GPS + TOLOSAT ORBIT PROPAGATION SCRIPT (SEGMENTED)
# ------------------------------------------------------------------------------
# This script propagates Tolosat and GPS satellites using TudatPy in segments
# (~94.4 min/orbit) to improve long-term integration stability.
#
## Initial States:
# - GPS satellite states are extracted from TLEs (≤5 days old) via `galileo_TLE_sync.py`
...
from gps_TLE_sync import gps_states_synced, gps_names, start_datetime
...
for gps in gps_names:
    acceleration_settings[gps] = {"Earth": [propagation_setup.acceleration.spherical_harmonic_gravity(2, 0)]}

all_spacecraft = ["Tolosat"] + gps_names
...
gps_states = gps_states_synced[["x", "y", "z", "vx", "vy", "vz"]].to_numpy()
initial_state = np.concatenate(([Tolosat_initial_state], gps_states)).flatten()
...
print("Propagating GPS + Tolosat in segments...")
...
print(f"Done! Saved all results in folder: gps_states/{next_index}")
