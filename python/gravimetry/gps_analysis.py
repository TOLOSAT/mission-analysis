...
results_dir = "results/1month"

# Choose time unit: "seconds", "hours", or "days"
time_unit = "days"  # Change this depending on your simulation
unit_factors = {
    "seconds": 1,
    "minutes": 60,
    "hours": 3600,
...
# Load data
# =============================
gps_visibility = pd.read_csv(os.path.join(results_dir, "gps_visibility.csv"), low_memory=False)
gps_windows = pd.read_csv(os.path.join(results_dir, "gps_windows.csv"), low_memory=False)
gps_sat_results = pd.read_csv(os.path.join(results_dir, "gps_sat_results.csv"), low_memory=False)
...
for style in ["dark", "light"]:
    ...
    axes[0].plot(gps_sat_results["seconds"] / time_factor, gps_sat_results["doppler_shift"] / 1e3)
    axes[0].set_title(f"Doppler shift of {selected_gps}")
    axes[0].set_xlabel(time_label)
    axes[0].set_ylabel("Doppler shift [kHz]")
    axes[0].set_xlim(0, gps_sat_results["seconds"].max() / time_factor)
    getattr(pf, f"finish_{style}_figure")(
        fig, os.path.join(results_dir, f"{selected_gps_nospace}_doppler_shift_{style}.png"), show=True
    )
...
for style in ["dark", "light"]:
    fig, axes = getattr(pf, f"{style}_figure")()
    axes[0].plot(gps_sat_results["seconds"] / time_factor, gps_sat_results["doppler_rate"])
    axes[0].set_title(f"Doppler rate of {selected_gps}")
    axes[0].set_xlabel(time_label)
    axes[0].set_ylabel("Doppler rate [Hz/s]")
    axes[0].set_xlim(0, gps_sat_results["seconds"].max() / time_factor)
    getattr(pf, f"finish_{style}_figure")(
        fig, os.path.join(results_dir, f"{selected_gps_nospace}_doppler_rate_{style}.png"), show=True
    )
...
for style in ["dark", "light"]:
    fig, axes = getattr(pf, f"{style}_figure")()
    axes[0].plot(gps_visibility["seconds"] / time_factor, gps_visibility["sum_ok"], linestyle="none", marker=".")
    axes[0].set_title("Visibility of gps satellites satisfying all 4 conditions")
    axes[0].set_xlabel(time_label)
    axes[0].set_ylabel("Number of satellites [-]")
    axes[0].set_xlim(0, gps_visibility["seconds"].max() / time_factor)
    getattr(pf, f"finish_{style}_figure")(
        fig, os.path.join(results_dir, f"gps_visibility_{style}.png"), show=True, force_y_int=True
    )
...
for style in ["dark", "light"]:
    fig, axes = getattr(pf, f"{style}_figure")()
    x_vals = gps_windows["seconds"] / time_factor
    y_vals = gps_windows["duration"] / 60
    ...
    getattr(pf, f"finish_{style}_figure")(
        fig, os.path.join(results_dir, f"gps_windows_{style}.png"), show=True
    )

# =============================
# Plot 5: True Visibility per PRN
# =============================
gps_prns_only = gps_visibility.select_dtypes(include=bool)
gps_visibility["time_unit_column"] = gps_visibility["seconds"] / time_factor
segments = []

def extract_inside_parentheses(label):
    if "(" in label and ")" in label:
        return label.split("(")[-1].split(")")[0]
    return label  # fallback si no hay paréntesis

prn_to_y = {prn: i for i, prn in enumerate(sorted(gps_prns_only.columns))}
yticklabels_clean = [extract_inside_parentheses(prn) for prn in prn_to_y.keys()]

for prn, y_val in prn_to_y.items():
    visibility = gps_prns_only[prn].astype(int)
    time = gps_visibility["time_unit_column"]
...
for style in ["dark", "light"]:
    ...
    axes[0].set_xlabel(time_label)
    axes[0].set_ylabel("GPS PRNs available")
    axes[0].set_title("True visibility duration of each GPS satellite")
    axes[0].set_yticks(list(prn_to_y.values()))
    axes[0].set_yticklabels(yticklabels_clean)
    axes[0].grid(True)
    getattr(pf, f"finish_{style}_figure")(
        fig, os.path.join(results_dir, f"gps_visibility_true_per_prn_{style}.png"), show=True
    )

# =============================
# Plot 6: Histogram of Window Durations
# =============================
durations_min = gps_windows["duration"] / 60  # seconds to minutes
...
# Compute total simulation duration (in days)
# Compute total simulation duration in seconds
total_time_seconds = gps_visibility["seconds"].max()
total_time_days = total_time_seconds / 86400

# Get all merged visibility intervals (seconds since start)
merged_segments = gps_windows[["start", "end"]].copy()
start_time = pd.to_datetime(gps_windows["start"].iloc[0])
...
all_durations = gps_windows["duration"].to_numpy()

# =============================
# Save stats
# =============================
with open(os.path.join(results_dir, "gps_visibility_stats.txt"), "w") as f:
    f.write("=== gps Visibility Stats ===\n")
    f.write(f"Total simulation duration: {total_time_days:.3f} days\n")
    f.write(f"Total time with visibility of at least one gps satellite (satisfying the 4 conditions): {total_unique_visibility_days:.3f} days\n")
