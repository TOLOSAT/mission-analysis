...
def generate_vts_file(
        epochs, vts_file_name, spacecraft_names=None, auto_start=False, VTS_path=None
):
    ...
    seconds = np.array(seconds) - 37  # TODO: Improve this to proper time system conversion
    ...
    for satellite_name in spacecraft_names:
        oem_file = f"{satellite_name}_POSITION_VELOCITY.TXT"
        aem_file = f"{satellite_name}_QUATERNION.TXT"
        if satellite_name == "TOLOSAT":
            add_satellite(xml, entities, satellite_name, oem_file, aem_file, model_name=TOLOSAT_MODEL)
        else:
            add_satellite(xml, entities, satellite_name, oem_file, aem_file)

    # Generate events and states
    generate_events(xml, project)
    generate_states(xml, project)
